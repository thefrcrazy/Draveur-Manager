...
import { Save, Clock, Zap, Play, Power, RotateCw, HardDrive, AlertCircle } from "lucide-react";
import { useLanguage } from "@/contexts/LanguageContext";
import { Schedule } from "./ServerSchedule";
import { Select, Checkbox, Input, Button, Modal } from "@/components/ui";
...
export default function ScheduleModal({ isOpen, onClose, onSave, schedule }: ScheduleModalProps) {
    ...
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title={schedule ? t("server_detail.schedule.edit_task") : t("server_detail.schedule.create_task")}
            footer={footer}
            size="lg"
        >
            ...
            <style>{`
                .schedule-form { display: flex; flex-direction: column; gap: var(--spacing-5); }
